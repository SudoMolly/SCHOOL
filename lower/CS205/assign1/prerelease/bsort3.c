...
// LEVEL 2 VERSION
// STRswap two int array elements
//
//void STRswap(int* a, int i, int j)
/*
void STRswap(char* a, int i, int j)
{
  if (i != j) {
    int tmp = a[i];
    a[i] = a[j];
...
// STRswaps the values of 2 given memory addresses, alloc'd as chars
//
//void STRswap(int* a, int i, int j)
//void STRswap(char* a, int i, int j)
void STRswap(char * i, char * j)
{
    //printf("%s", "ENTER STRswap\ni: ");
    //printf("%c", *i, 
  if (*i != *j) 
  {
    char tmp = *i;
...
//         counter to help STRswap btw/n current row and next row, and j is
